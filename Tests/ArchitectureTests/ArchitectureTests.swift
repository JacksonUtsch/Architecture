...
final class ArchitectureTests: XCTestCase {
    let testStore = TestStore(initialState: TestState(), reducer: testReducer(state:action:env:), environment: EmptyEnvironment())
    ...
    func testEffect() {
        testStore
            .assert(.chain, that: {$0.name == "custom name" && $0.number == 1}, with: 0.01)
    }
    ...
    func testScope() {
        let subStore = testStore.scope(localEnvironment: EmptyEnvironment(), toLocalState: {$0.substate}, fromLocalAction: { TestAction.substate($0) })
        
        subStore.assert(.insert(Substate.IdentifiableInt(value: 5)), that: {$0.contents.collection.count == 2})
        testStore.assert(that: {$0.substate.contents.collection.count == 2})
    }
}
